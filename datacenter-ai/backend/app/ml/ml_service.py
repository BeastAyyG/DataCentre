...
class MLService:
    ...
    def load_models(self) -> None:
        ...
        self._if_model = IsolationForestModel(self.artifacts_dir / "isolation_forest_v1.joblib", scaler_path)
        self._lstm_model = LSTMAutoencoder(self.artifacts_dir / "lstm_autoencoder_v1.pt", lstm_scaler_path)
        self._xgb_model = XGBAnomalyScorer(self.artifacts_dir / "xgb_anomaly_v1.json")
        self._catboost_model = CatBoostClassifier(self.artifacts_dir / "catboost_classifier_v1.cbm")
        self._prophet_model = ProphetForecaster(self.artifacts_dir / "prophet_temp_v1.joblib")
            ...
            for i, dev in enumerate(devices):
                ...
                for j in range(12):
                    if j < len(history_orm):
                        sequence_matrix[i][j] = self._build_feature_vector(history_orm[j]).flatten()
                    else:
    ...
    def _get_device_history(
        self, db, device_id: str, limit: int = 100
    ) -> pd.DataFrame:
        rows = (
            db.query(SensorReading)
        ...
        with get_db_context() as db:
            ...
            if history.empty:
                history = pd.DataFrame({
                    "ds": pd.date_range(start=datetime.utcnow() - timedelta(hours=1), periods=12, freq="5T"),
                    "y": np.random.normal(22, 1, 12),
                    "cooling_setpoint_c": [22.0] * 12,
                    "hour_of_day": [t.hour for t in pd.date_range(start=datetime.utcnow() - timedelta(hours=1), periods=12, freq="5T")],
                    "day_of_week": [t.weekday() for t in pd.date_range(start=datetime.utcnow() - timedelta(hours=1), periods=12, freq="5T")],
                })
                ...
                "forecast_series": [
                    {
                        ...
                        "scenario_temp_c": round(float(scenario_df.iloc[idx]["yhat"]), 2)
                        if idx < len(scenario_df) else round(float(row["yhat"]), 2),
                    }
