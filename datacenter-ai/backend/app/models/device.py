...
class Device(Base):
    ...
    datacenter_id: Mapped[str] = mapped_column(
        String(36), ForeignKey("datacenters.id"), nullable=False
    )
    name: Mapped[str] = mapped_column(String(100), nullable=False)
    type: Mapped[str] = mapped_column(
        String(50), nullable=False
    )  # rack, pdu, crac, sensor, network
    ...
    )  # healthy, at_risk, critical, offline
    ...
    anomaly_alerts = relationship(
        "AnomalyAlert", back_populates="device", cascade="all, delete-orphan"
    )
