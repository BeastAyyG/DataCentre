...
# fix registry
Path("model_registry.json").write_text(json.dumps({"models":[]}))

cols = ['inlet_temp_c', 'outlet_temp_c', 'power_kw', 'airflow_cfm', 'humidity_pct', 'cpu_util_pct']
...
if train_path.exists() and test_path.exists() and label_path.exists():
    ...
    df_hot['timestamp'] = [start_time + pd.Timedelta(minutes=5*i) for i in range(len(df_hot))]
